...
class PostController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if($request->hasFile('featured_image')){
            ...
            Storage::disk('public')->put("images/{$user->id}", $featuredImage);
            $post->featured_image=url('/')."/images/{$user->id}/".$filename;
    ...
    public function update(Request $request, $id)
    {
        if(Auth::user()){
            ...
            if($request->hasFile('featured_image')){
                $featuredImage=$request->file('featured_image');
                $filename = time().$featuredImage->getClientOriginalName();
                Storage::disk('public')->put("images/{$post->id}", $featuredImage);
                $post->featured_image=url('/')."/images/{$post->id}/".$filename;
            }
